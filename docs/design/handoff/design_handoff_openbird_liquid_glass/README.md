...
> `docs/privacy-routes.yaml` as authoritative for current privacy copy.
...
- **Layout:** 540px glass sheet (traffic lights). Centered: 54px bird, "Welcome to OpenBird", privacy subhead ("stores text, not screenshots or images"). Four permission rows (icon tile in `rgba(47,127,242,0.14)`, title + subtitle, status): Screen text capture **Granted** · Accessibility **Granted** · System audio for meetings **Enable** button · Local model · Ollama **Connected**. Primary "Start capturing" button. Footer lock line: "Local by default. Pause anytime from the menu bar."
