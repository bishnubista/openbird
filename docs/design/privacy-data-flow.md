...
| `chat.founder_context` | explicit founder recap question plus bounded recent capture, OCR, meeting, file-ingest, and local filesystem-MCP occurrences | question, at most eight exclusion-filtered occurrence chunks, occurrence citations, aggregate retrieval counts | chat output is not persisted | inherits the configured active model route; remote use still requires cloud opt-in | deleting/pruning observations removes future retrievability and invalidates the evaluator snapshot | lead Ask suggestion, citation chips, route label |
...
| `eval.founder_context_capture` | scheduled aggregate SQL + metadata sidecars/files | app bundle ids observed in stored occurrences, closed source/mode/outcome counts, timestamps, corpus/storage/resource counts, readiness reason codes | one atomic 0600 JSON snapshot in the data-dir logs folder; aggregate read + replacement share a short DB writer lock with deletion; manual probe fields are transient only | none; scheduled command constructs no model/embed/rerank provider | selective/full delete and prune must invalidate the snapshot before commit or roll back; serialization prevents stale post-delete replacement; data-dir purge removes it | `openbird eval founder-context run`, optional six-hour LaunchAgent |
...
9. Scheduled founder-context evaluation may read captured text only inside
   aggregate SQL expressions. Its process output and snapshot must contain no raw
   text, excerpts, titles, URLs, prompts, answers, or capture-attempt bundle ids.
