...
| `chat.day_facts` | deterministic day-fact answer branch | question used only for classification, local day-memory productivity facts, derived citation source ids, memory-context counts | SQLite `day_memories`; chat output is not persisted | no completion request; the chat entrypoint may construct a provider, but this branch returns before `provider.complete`, so no day-fact data egresses | purge/prune removes source observations and derived day memory, removing future retrievability | `reasoning_route=local_deterministic` / deterministic answer label |
