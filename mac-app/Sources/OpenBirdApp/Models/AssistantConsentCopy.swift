...
enum AssistantConsentCopy {
    ...
    static let claudeConnectMessage =
        ...
        + "memory-store totals, encryption state, exclusion counts, the queried time "
        + "window with its timezone, and a host label identifying this Mac. Those "
        + "results are sent to Anthropic and cannot be recalled by deleting local "
        + "memory. OpenBird never sends data in the background."
    ...
    static let chatGPTConnectMessage =
        ...
        + "encryption state, exclusion counts, the queried time window with its "
        + "timezone, and a host label identifying this Mac leave only when you ask "
        + "ChatGPT to use an OpenBird tool, and cannot be recalled by deleting local "
        + "memory."
}
